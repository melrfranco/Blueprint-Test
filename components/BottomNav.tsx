...
import { HomeIcon, UsersIcon, CalendarIcon, DocumentTextIcon, SettingsIcon, CheckCircleIcon, PlusIcon } from './icons';
...
const BottomNav: React.FC<BottomNavProps> = ({ role, activeTab, onNavigate }) => {
  ...
  const clientNav = [
    { name: 'My Plan', icon: DocumentTextIcon, key: 'plan' },
      ...
      {navItems.map(item => (
        <button
          ...
          className={`flex flex-col items-center justify-center w-full p-1 rounded-lg transition-colors ${
            item.isAction 
             ? 'text-brand-pink -mt-6' 
             : activeTab === item.key || (activeTab === 'home' && item.key === 'dashboard') 
                ? 'text-brand-pink' 
                : 'text-gray-700 hover:bg-gray-100 hover:text-gray-950'
          }`}
        >
          {item.isAction ? (
              <div className="bg-brand-pink text-white rounded-full p-3 shadow-lg border-4 border-gray-900">
                  <item.icon className="h-6 w-6" />
              </div>
          ) : (
              <item.icon className={`h-7 w-7 mb-1 ${activeTab === item.key ? 'stroke-[3]' : 'stroke-[2.5]'}`} />
          )}
          
          <span className={`text-[12px] font-black uppercase tracking-tight ${item.isAction ? 'mt-1 text-brand-pink' : (activeTab === item.key || (activeTab === 'home' && item.key === 'dashboard') ? 'text-brand-pink' : 'text-gray-950')}`}>
              {item.name}
          </span>
        </button>
      ))}
