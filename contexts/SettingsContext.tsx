...
export const SettingsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [membershipConfig, setMembershipConfig] = useState<MembershipConfig>(() => {
    try {
      ...
      return { enabled: true, tiers: MEMBERSHIP_TIERS };
    }
  });
  ...
  const [integration, setIntegration] = useState<IntegrationSettings>(() => {
    try {
      ...
      return { provider: 'square', environment: 'production' };
    }
  });

  const [textSize, setTextSize] = useState<AppTextSize>(
    () => (localStorage.getItem('admin_text_size') as AppTextSize) || 'M'
  );
  const [pushAlertsEnabled, setPushAlertsEnabled] = useState<boolean>(
    () => localStorage.getItem('admin_push_alerts_enabled') === 'true'
  );
  ...
  useEffect(() => {
    ...
    const loadDataForUser = async (user: any) => {
      ...
      try {
        const { data: teamData, error: teamFetchError } = await supabase.from('square_team_members').select('*');

        if (teamFetchError) throw teamFetchError;

        if (!teamData || teamData.length === 0) {
          throw new Error('No team members found. Sync your team from Square in Settings.');
        }
  ...
  const createClient = async (clientData: { name: string; email: string }): Promise<Client> => {
    ...
    const { data, error } = await supabase
      .from('clients')
      .insert({ name: clientData.name, email: clientData.email, avatar_url } as any)
      .select()
      .single();
    ...
    if (!data) throw new Error('Client creation failed: no data returned.');
  ...
  const resolveClientByExternalId = async (
    ...
    const { data: newDbClient, error: createError } = await supabase
      ...
      .insert({
        ...
        avatar_url:
          clientDetails.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(clientDetails.name)}&background=random`,
      } as any)
      .select()
      .single();

    if (createError) throw createError;
    if (!newDbClient) throw new Error('Client creation failed: no data returned.');

    const newClient: Client = {
      id: (newDbClient as any).id,
      externalId: (newDbClient as any).external_id,
      ...(isValidUUID((newDbClient as any).id) ? {} : (() => { throw new Error('Invalid client UUID returned from DB'); })()),
  ...
  const saveAll = async () => {
    ...
    const settingsBlob = {
      ...
      linkingConfig,
      levels,
      clients: clients.filter(c => isValidUUID(c.id)),
      membershipConfig,
      branding,
      integration,
      textSize,
    ...
    const { error } = await supabase
      .from('merchant_settings')
      .upsert(
        { merchant_id: merchantId, settings: settingsBlob, updated_at: new Date().toISOString() } as any,
